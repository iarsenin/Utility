...
<main>
  ...
  <section class="abstract">
    <h2>Abstract</h2>
    <p>Modern environments can change subjective payoffs faster than the material capacities behind future choices can adjust. A recommender system, market interface, peer group, institution, or AI product may raise the short-run value of a substitute behavior. That behavior can then change the stock that makes tomorrow's alternatives feasible: friendship skill, solvency, metabolic resilience, learning, trust, or institutional competence.</p>
    <p>The capacity loop has three steps plus a selection layer. Exposure \(z\) and capacity \(K\) form a fast preference state or induced payoff map; agents choose inside that induced game, with Nash equilibrium as the strategic consistency condition; the resulting behavior moves the slower capacity law. In the scalar shortcut, the fast step maps directly into substitute share, \(p(K,z)=\sigma\{\beta(q+z-\rho K)\}\), and capacity drift is \(\Phi(K)=\alpha+rK^2(1-K)-dK-Lp(K,z)\). The endogenous part of the local multiplier is the chain from capacity to payoff, payoff to behavior, and behavior back to capacity.</p>
    <p>This normal form produces three long-run patterns. With weak damage or strong repair, capacity recovers. In the baseline trap calibration, the stable low-capacity state is near \(K=0.085\), the unstable threshold is near \(K=0.212\), and the stable high-capacity state is near \(K=0.843\). In the random audit, the collapse-prone class reaches the lower boundary in 88.5 percent of draws and has no interior stable state in 15.4 percent. In all patterns, choice can remain Nash-consistent after subjective payoffs are formed.</p>
    <p>The correction result is sharper than the usual intuition that current feedback eventually corrects behavior. Adding a signal proportional to current capacity drift, \(\chi\Phi_\chi(K)\), changes speeds and slopes but leaves the interior steady-state capacities unchanged, because the signal is zero at any interior steady state. In the diagnostics here, a sufficiently strong level-of-capacity channel can move the steady states and remove the low trap; competition can also help when the competitive score rewards material viability. If the score rewards engagement instead, the higher-scoring rule can gain relative share while aggregate material scale falls.</p>
    <p>The empirical content is a set of observables: the payoff-forming exposure, the substitute behavior, the material capacity law, and the competitive score. AI companions, sports betting, hyper-palatable food, appetite-changing medicines, dating retreat, appearance markets, and political outrage differ in substance, but they become comparable once those four objects are measured.</p>
  </section>

  <section>
    <h2>1. The Puzzle In Plain Words</h2>
    <p class="lede">People can make coherent choices inside a payoff criterion that the environment helped produce. The choice can be rational and sincere at the moment of choice while weakening the slower capacity that would make a different future choice feasible.</p>

    <p>An isolated adult may prefer an AI companion tonight because it is responsive, low-risk, and always available. A bettor may want the next in-game wager because the app makes risk feel like part of the match. A teenager may retreat from courtship because digital substitutes feel safer. A citizen may prefer outrage media because a familiar personality feels more fluent than slow verification. These choices need not be mistakes at the moment they are made. The sharper issue is that the environment helped form the criterion used to choose.</p>
    ...
    <p>The model treats subjective payoffs as operative, not illusory. The question is how the payoff-forming system changes later capacity, and whether competition selects systems that preserve that capacity.</p>
    ...
    <div class="result-list">
      ...
      <ol>
        ...
        <li><strong>Collapse.</strong> In the random audit, higher-damage feedback often pushes the projected dynamic to the lower boundary, and some draws have no interior stable capacity state.</li>
        <li><strong>Alarm.</strong> In the one-capacity logit model, a warning tied only to current decline changes the speed of motion but does not move the interior steady-state capacities.</li>
        <li><strong>Selection.</strong> Competition selects the rule with the higher score. It preserves capacity when the score ranks the capacity-preserving rule above the sink; material viability is one aligned score.</li>
      </ol>
    </div>
  </section>
  ...
  <section>
    ...
    <p>The local feedback multiplier is the mathematical version of this story. Its direct terms capture how capacity moves mechanically. Its endogenous preference component is the chain in which capacity affects the fast payoff state, the payoff state affects behavior, and behavior affects capacity. When that component reinforces repair, the loop tends to correct itself. When it is sufficiently positive around the middle of the state space, the same model can produce a threshold: initial states above the threshold move toward the high-capacity attractor, while states below it move toward the low-capacity attractor.</p>
  ...
  <section>
    ...
    <h3>Collapse</h3>
    <p>The audit also contains collapse-prone dynamics. In those parameter regions, higher damage and sensitivity often push the projected state to the lower boundary, and some draws have no interior stable capacity state. In this model, recovery from such a region occurs only when the forces pushing capacity up or down change. In a betting case, this is the point where solvency is so damaged that ordinary budgeting no longer restores the previous option set.</p>
    ...
    <p>Not automatically. The model separates two signals. One signal says, "this is getting worse right now." In the one-capacity diagnostic, that current-drift signal can slow the path, but it does not change the interior steady-state capacities. At an interior steady state there is no current drift, so the signal goes quiet.</p>
    ...
    <p>Competition is Darwinian in the precise sense of a share dynamic over preference-forming rules. It does not make the outcome benevolent. In the numerical diagnostic, a bridge design and a sink design start with equal shares. If the competitive score is material viability, the bridge expands and total scale recovers. If the score is engagement, the sink expands and total scale falls. Relative victory and absolute survival are different equations.</p>
  ...
  <section>
    ...
    <p>Competition decides which payoff-forming systems spread only after the score is specified. The share equation is governed by a relative score \(S\). Aggregate survival is governed by a material growth rate \(g\). If \(S\) rewards engagement while \(g\) tracks solvency, health, trust, or social capacity, a rule can win relative share while the carrier population, capital stock, or institution shrinks.</p>
    ...
    <p>This disciplines the Darwinian language. Fitness has no content until the metric is specified: reproduction, solvency, health, learning, institutional continuity, verification capacity, or another material metric. Different metrics can disagree. That disagreement is part of the economics.</p>
  ...
  <section>
    ...
    <p>Competition is the Darwinian part. Imagine two companion systems. A health insurer, school, employer, clinical trial, or public agency rewards the bridge when offline contact or social anxiety improves. A platform ranking system rewards the sink when retention, session length, paid attachment, or daily dependence rises. Material-viability competition expands the bridge if it builds long-run capacity. Engagement competition expands the sink if it captures more time, even when human social capacity falls. Under this model, design and selection metric determine which companion system gains share.</p>

    <p>The empirical design is concrete. Randomized or instrumented exposure to companion designs can help identify the first link under the usual design assumptions. Short-run loneliness and longer-run offline contact capacity show whether the substitute changes the stock. Product survival, funding, ranking, or adoption data reveal the competitive score. A fixed-preference account fits when exposure changes only convenience or information. The feedback account gains force when exposure changes the payoff of human contact and behavior then changes later social capacity.</p>
  </section>
  ...
  <section>
    ...
    <p>A fixed-preference account is adequate when exposure changes convenience, information, or prices but not the payoff map, or when behavior does not feed back into future capacity. A feedback account gains force when exposure changes payoff, payoff changes behavior, and behavior changes the future capacity that shapes later payoff.</p>
  ...
  <section>
    ...
    <p>The model also separates weak optimism from changed dynamics. Current-decline warnings below a threshold need not move the state in the one-capacity model. The low-capacity equilibrium can absorb the warning into the same payoff that created the trap. In this model, correction occurs through a changed force, a threshold crossing, or competition that moves mass toward a more viable rule.</p>
  </section>
  ...
  <section>
    ...
    <p>Fast subjective payoff formation plus slow capacity feedback yields bridge, threshold-trap, and collapse-prone dynamics in the scalar diagnostics. A warning tied only to current decline does not move interior steady-state capacities. A sufficiently strong level-of-capacity channel can remove the low trap in the diagnostic calibration. Competition can shift composition toward the capacity-preserving rule when the competitive score ranks that rule above the sink; the material-viability diagnostic is one aligned case.</p>
