...
def build_article() -> str:
    ...
    stock_summary = read_csv(ROOT / "results" / "tables" / "self_correction_stock_feedback_summary.csv")
    competition_summary = read_csv(ROOT / "results" / "tables" / "self_correction_competition_summary.csv")
    ...
    stock_summary_table = html_table(
        [
            row
        ...
        [
            ("capacity_protection", "Capacity protection rho"),
            ("stable_count", "Stable states"),
            ("low_trap_present", "Low trap"),
            ("threshold_present", "Threshold"),
            ("high_state_present", "High state"),
        ],
        {"capacity_protection", "stable_count"},
    )
    competition_table = html_table(
        competition_summary,
        [
            ("selection_metric", "Competitive score"),
            ("competition_intensity", "Competition omega"),
            ("final_bridge_share", "Final bridge share"),
            ("final_population_mass", "Final population mass"),
...
<main>
  ...
  <section class="abstract">
    <h2>Abstract</h2>
    <p>Modern environments can change what people find rewarding faster than the relevant real-world capacities can adjust. A recommender system, peer group, market interface, institution, or AI product may make one action feel best today. That action can then change the stock that makes tomorrow's alternatives feasible: friendship skill, solvency, metabolic resilience, learning, trust, family agency, or institutional competence.</p>
    <p>The result is a classification in a minimal fast-preference model. Under explicit conditions, some loops repair themselves. Some create two durable states: a low-capacity state where the substitute remains attractive, and a high-capacity state where the substitute loses its pull. A threshold separates them. In more severe cases, the high-capacity state can disappear. None of these outcomes requires irrational choice. Agents can optimize coherently inside the payoff criterion formed at the moment of choice.</p>
    <p>The model also separates two kinds of correction. A signal that only says "the current path is deteriorating" does not remove a bad interior steady state in the one-capacity example; at a steady state that signal is silent. In the mechanisms studied here, durable correction comes from a persistent link from material capacity back into what feels rewarding, or from competition under a score that preserves or reproduces the named capacity. If competition rewards engagement instead, a high-engagement rule can expand while the material stock falls.</p>
    <p>The framework is meant as a disciplined test, not a one-cause theory of modern life. For AI companions, sports betting, hyper-palatable food, appetite-changing medicines, dating retreat, appearance markets, and political outrage, the same questions become empirical: what forms the payoff used at choice time, what substitute behavior is selected, what material capacity changes, and what competitive score decides which rule survives?</p>
  </section>

  <section>
    <h2>1. The Puzzle In Plain Words</h2>
    <p class="lede">People can make coherent choices inside a payoff criterion that the environment helped produce. The choice can be rational, sincere, and stable in the short run while weakening the slower capacity that would make a different life possible.</p>

    <p>An isolated adult may prefer an AI companion tonight because it is responsive, low-risk, and always available. A bettor may value the next in-game wager because the interface makes participation feel immediate. A teenager may retreat from courtship into safer digital substitutes. A citizen may prefer outrage media because a familiar personality feels more fluent than institutional verification. These choices need not be mistakes at the moment of choice. The sharper issue is that the environment is helping form the criterion used to choose.</p>

    <p>The framework distinguishes three objects. A <em>subjective payoff</em> is the criterion the person or organization uses at choice time. A <em>substitute behavior</em> is an action that offers immediate reward or relief while possibly replacing practice of another capacity. A <em>material capacity</em> is the slower stock that makes future alternatives feasible: social skill, friendship, solvency, metabolic health, learning, trust, or institutional competence. A <em>material outcome metric</em> says which of those stocks matters in the application.</p>

    <p>The central question is not whether the subjective payoff is fake. In economic terms it is real if it guides choice. The central question is whether the system that forms that payoff builds or depletes the material capacity being evaluated, and whether competition selects systems that preserve that capacity.</p>

    <div class="result-list">
      <h3>Results</h3>
      <ol>
        <li><strong>Fast preference formation changes the game.</strong> Nash equilibrium is still the right consistency test after payoffs are formed: no player wants to change strategy given the induced payoffs and the others' strategies. It is not a certificate that the payoff-forming environment is good.</li>
        <li><strong>Capacity feedback creates three qualitative regimes.</strong> The system can self-correct, settle into a low-capacity trap separated from a high-capacity state by a threshold, or lose the high-capacity state when damage overwhelms repair. A bad long-run outcome can therefore be locally rational.</li>
        <li><strong>Not all reality signals repair the loop.</strong> In the one-capacity example, an instantaneous current-drift signal can change adjustment speeds without moving the interior steady states. A persistent material-capacity channel can remove the low trap in the diagnostic calibration.</li>
        <li><strong>Competition selects whatever is scored.</strong> If the score is material viability, a capacity-building rule can expand. If the score is engagement, a capacity-depleting rule can win while the total scale of the population, capital stock, or institution carrying the rule falls.</li>
        <li><strong>The examples are tests, not proof by anecdote.</strong> Each application must measure exposure, behavior, capacity, and the competitive score before the mechanism is established.</li>
      </ol>
    </div>
  </section>
  ...
  <section>
    ...
    <p>The model has two clocks. On the fast clock, a preference-forming rule maps exposure and current capacity into the subjective payoff used for choice. On the slow clock, the chosen behavior updates the material capacity. No assumption is made that all institutions, populations, or cultures are slow. If an institutional rule changes as quickly as the payoff state, it belongs in the fast environment. If it changes with capacity, it belongs in the slow state. The empirical task is to estimate the clocks.</p>

    <div class="result-strip">
      <div><strong>Exposure</strong><span>A feed, product, peer norm, interface, institution, or AI system changes what feels rewarding.</span></div>
    ...
    <p>The one-capacity example in the appendix uses a normalized material capacity. High capacity makes the non-substitute life more feasible and therefore reduces the substitute's pull. Substitute use can damage or crowd out capacity. Repair can rebuild capacity, but repair is weak near the bottom because the person or system lacks the stock needed to practice the capacity. That small asymmetry is what allows a threshold trap. In the AI companion case, the same logic asks whether synthetic interaction builds enough social capacity to make ordinary contact easier later, or replaces the practice needed for that contact.</p>

    <p>Competition enters after the competitive score has been named. A Darwinian statement is incomplete until the score is explicit. A rule may win because it improves material viability, meaning it preserves or reproduces the named capacity, because it captures attention, because it raises short-run revenue, because it reproduces biologically, or because it persists institutionally. These scores can agree, but they need not. The formal model therefore separates relative prevalence from absolute material survival.</p>
  </section>

  <section>
    <h2>3. What The Model Says</h2>
    <p>The first result is reassuring. Fast preference formation does not automatically produce damage. In the one-capacity model, if the substitute is mild, repair is strong, or capacity remains high enough to protect the outside option, the system converges to a high-capacity state. In an AI companion example, the companion is a bridge if it lowers anxiety and helps the person return to ordinary human contact with greater capacity.</p>

    <p>The second result is the trap. Low capacity can make the substitute attractive, and heavy substitute use can prevent capacity from rebuilding. High capacity can also be stable: because the outside option is feasible, the substitute is less tempting, and lower substitute use lets capacity persist. Between them is a threshold. Above it, the same environment can converge upward; below it, the same environment can converge downward.</p>

    <p>The third result is more severe. A system may not merely become harder to rescue; the high-capacity steady state can disappear. That distinction matters empirically. A trapped system has a recoverable upper state but needs a threshold crossing. A collapse-prone system has lost that upper state under the current environment and needs the forces pushing capacity up or down to change.</p>

    <figure>
      <img src="../results/figures/material_feedback_paths.svg" alt="Capacity paths from different starting values">
      <figcaption>Threshold diagnostic. Same environment, different starting capacity, different long-run outcome: below the threshold the path falls to the low-capacity state; above it the path rises to the high-capacity state. A repair intervention can move the same low starting point into the high-capacity basin.</figcaption>
    </figure>

    <div class="reader-result">
      <strong>What changes objective prediction.</strong> In this framework, a change has objective force when it changes the capacity path, moves the state across a threshold, removes a bad attracting state, or changes the competitive score that selects among preference-forming rules. Warnings, moral appeals, and information campaigns are not classified by their intent; they are classified by whether they change those dynamics.
    </div>

    <h3>Does Reality Correct The System Automatically?</h3>
    <p>Not automatically. The model distinguishes an instantaneous current-drift signal from a persistent capacity signal. A current-drift signal is like an alarm that responds to the direction of movement: "this is getting worse right now." At a steady state in the model there is no movement, so the alarm has no force left. It can damp the path but does not remove the bad steady state.</p>

    <p>A persistent capacity signal is different. It says: "the level of capacity itself changes how attractive the substitute feels." That channel can remove a low-capacity trap because it changes what feels rewarding even when the system is not currently moving. In the diagnostic calibration, this is the first tested repair channel that changes the number and location of long-run states.</p>

    <figure>
      <img src="../results/figures/self_correction_channels.svg" alt="Self-correction channel diagnostics">
      <figcaption>Two correction channels. A current-drift signal can change the speed of movement but leaves the same interior long-run destinations in this diagnostic. A persistent capacity-stock signal can change those destinations and remove the low-capacity trap. The distinction matters because transient regret or pain often behaves like the first channel, while institutions, skills, balance sheets, health, and social networks behave like the second.</figcaption>
    </figure>

    <h3>Competition Is Corrective Only Under The Right Metric</h3>
    <p>Competition is Darwinian, but Darwinian does not mean benevolent. In the numerical diagnostic, a bridge rule and a sink rule start at the same capacity and equal shares. When competition rewards material viability, the bridge expands and the total scale of the population, capital stock, or institution carrying the rule recovers. When competition rewards engagement, the sink expands and that total scale falls. The same competitive process does both. The difference is the score being rewarded.</p>

    <figure>
      <img src="../results/figures/competition_selection_channels.svg" alt="Competition selection channels">
      <figcaption>Competition over rules. The bridge design preserves capacity; the sink design captures substitute use. When the competitive score is material viability, the bridge takes over. When the score is engagement, the sink wins even though total surviving scale falls sharply. The figure separates relative victory from absolute survival.</figcaption>
    </figure>
  </section>

  <section>
    <h2>4. Nash Equilibrium And Competition</h2>
    <p>Nash does one job here. After the preference-forming rule creates subjective payoffs, Nash equilibrium asks whether each player is choosing coherently given those payoffs and the others' choices. The equilibrium can be fully coherent while the material capacity being evaluated moves in a bad direction.</p>

    <p>Competition decides which preference-forming systems spread, but only after the model says what competition rewards. If the score rewards preserving or reproducing the named capacity, rules with lower viability lose prevalence. If the score is engagement, attention, status, or short-run revenue, the rule that expands may be the one that damages material capacity. This is why "competition will fix it" is not a general result. The claim needs the score.</p>

    <div class="reader-result">
      <strong>Selection claim.</strong> Relative survival depends on the competitive score. Absolute survival depends on the material growth of the population, capital, or institution carrying the rule. If every available rule depletes the relevant capacity, the long-run conclusion can be disappearance rather than convergence to a fitter taste.
    </div>

    <p>This is where the Darwinian language becomes disciplined. "Fitness" is not a loose metaphor. It must be named: reproduction, solvency, health, learning, institutional continuity, verification capacity, or some other material outcome metric. Different metrics can disagree, and the disagreement is part of the economics.</p>

    <p>Political outrage makes the strategic point concrete. If each broadcaster, politician, or influencer expects restraint to lose attention under the induced payoff, escalation can be a best response for each actor. Nash equilibrium explains why the escalation can be stable after the payoffs are formed. Competition explains why the attention-scoring environment itself may spread, even while the material capacity being depleted is verification, trust, or correction acceptance.</p>
  </section>

  <section>
    <h2>5. A Full Example: AI Companions And Loneliness</h2>
    <p>The cleanest way to read the model is through one case. The <a href="#ref-who-loneliness">WHO Commission on Social Connection</a> and the <a href="#ref-surgeon-general">US Surgeon General</a> treat loneliness as a health and community problem, while <a href="#ref-hbs-ai">De Freitas et al.</a>, <a href="#ref-common-sense-ai">Common Sense Media</a>, and <a href="#ref-ai-companions">Malfacini</a> document the emerging AI-companion setting. Suppose an AI companion is always available, responsive, nonjudgmental, and tuned by a platform. The exposure is the availability and emotional fluency of the companion. The subjective payoff is the immediate value of synthetic interaction relative to costly human contact. The substitute behavior is time spent in the synthetic relationship instead of attempts at ordinary social contact.</p>

    <p>The material capacity is not "happiness" in general. It is a measurable stock: offline contact attempts, friendship repair attempts, invitations accepted, social anxiety after repeated use, time spent in mixed social settings, or durable human ties. A bridge design lowers the cost of contact while preserving or increasing those stocks. A sink design supplies enough synthetic reward that the outside stock decays. The same user can make locally rational choices under either design; the objective difference is the path of measured social capacity.</p>

    <p>The threshold result says that small changes may have different effects depending on the starting stock. Above the threshold, synthetic interaction can be a scaffold that reduces anxiety and returns the user to human contact with higher capacity. Below the threshold, the same local reward can stabilize retreat if outside contact becomes too costly relative to the induced payoff. The model does not say loneliness vanishes. It says an alarm based only on "am I getting worse right now?" can go quiet once a low-contact routine becomes stable.</p>

    <p>Competition is the Darwinian part. Imagine two companion systems. A health insurer, school, employer, clinical trial, or public agency rewards the bridge design when later offline contact, friendship repair attempts, or social anxiety improve. A platform ranking system, venture dashboard, or creator marketplace rewards the sink design when retention, session length, paid attachment, or daily dependence rises. Material-viability competition expands the bridge if it produces higher long-run capacity. Engagement competition expands the sink if it captures more time, even when the absolute stock of human social capacity falls. This is not a claim that AI companionship is intrinsically harmful or beneficial; it is a claim that the competitive score determines which design survives.</p>

    <p>The empirical test is therefore concrete: randomize or instrument exposure to companion designs, measure immediate loneliness and longer-run offline contact capacity, and observe which competitive score governs product survival. A fixed-preference account is adequate if the exposure changes only convenience or information. The feedback account is needed if exposure changes the payoff of human contact and the resulting behavior changes later social capacity.</p>
  </section>

  <section>
    <h2>6. Candidate Domains</h2>
    <p>The same mechanism can be tested elsewhere, but the following table is a research menu, not a claim that one model explains everything. Each row should be read as a falsifiable mapping, not as a claim already proven.</p>

    <p>Sports betting illustrates the competition point: handle, retention, and in-game engagement can rise while household solvency falls. A fixed-preference story can describe gambling demand; the feedback story asks whether the interface changes the payoff of risk and whether repeated play changes future solvency.</p>

    <p>GLP-1 medicines illustrate the positive case. A fast change in appetite payoff may become a bridge if it rebuilds durable metabolic capacity. It becomes a sink only if the induced payoff produces indefinite product dependence without rebuilding the stock that makes healthier eating and weight maintenance feasible outside the product.</p>

    <p>Political outrage illustrates strategic stability: escalation can be individually rational after the attention payoff is formed while degrading verification capacity. A fixed-preference account can describe demand for partisan media; the feedback account asks whether the media environment changes the payoff of outrage and whether repeated exposure lowers later correction acceptance.</p>

    <p>Each row must name a substitute, a measurable capacity, and a competitive score before it becomes evidence.</p>

    <p class="table-title">Candidate tests of the mechanism.</p>
    <table>
      <thead>
        <tr><th>Domain</th><th>Substitute</th><th>Measurable capacity</th><th>Preference-forming rule</th><th>Competitive score to check</th></tr>
      </thead>
      <tbody>
        <tr><td>Dating retreat</td><td>Avoidance, pornography, games, low-risk digital interaction</td><td>Mixed social-network size, date initiation attempts, rejection anxiety, relationship formation</td><td>Rejection avoidance and online substitutes raise the subjective payoff of non-courtship.</td><td>Retention in substitutes versus later courtship capacity.</td></tr>
        <tr><td>Sports betting and debt</td><td>Micro-wagers, frictionless checkout, identity purchases</td><td>Savings buffer, credit utilization, missed payments, debt service</td><td>Frictionless risk turns fandom or shopping into repeated immediate reward.</td><td>Handle, revenue, and attention versus household solvency.</td></tr>
        <tr><td>Hyper-palatable food</td><td>Ultra-processed, high-reward consumption</td><td>Weight trajectory, metabolic markers, appetite regulation, food cue sensitivity</td><td>Food environments raise the subjective reward of repeated consumption.</td><td>Sales, attention, and repeat purchase versus metabolic capacity.</td></tr>
        <tr><td>GLP-1 repair channel</td><td>Appetite-changing medicine as a bridge intervention</td><td>Metabolic markers, appetite regulation after withdrawal, durable behavior change</td><td>Medicine changes the reward field and may shift the capacity path.</td><td>Durable metabolic capacity versus continued product dependence.</td></tr>
        <tr><td>Appearance arms races</td><td>Filters, cosmetic escalation, lookmaxxing advice</td><td>Body satisfaction, procedure spending, baseline acceptability after exposure</td><td>Ranked visual comparison makes yesterday's enhancement today's baseline.</td><td>Status attention versus financial and identity resilience.</td></tr>
        <tr><td>Political outrage</td><td>Outrage feeds, parasocial news trust</td><td>Correction acceptance, institutional trust, cross-cutting exposure</td><td>Feeds and personalities make anger and familiarity more rewarding than verification.</td><td>Engagement and audience share versus verification capacity.</td></tr>
      </tbody>
    </table>

    <p>The table deliberately uses measurable stocks rather than moral language. For sports betting, the object is not whether wagering is virtuous; it is whether repeated exposure changes the payoff of risk and whether behavior predicts later solvency measures. For food, the object is whether a reward environment or medicine changes durable metabolic capacity. For politics, the object is not whether anger is justified; it is whether the induced media game lowers correction acceptance or institutional trust. The final column is the main contribution: it names what competition is rewarding, so the survival claim can be checked rather than assumed.</p>

    <p class="small">Empirical anchors for these rows include <a href="#ref-ueda">Ueda et al.</a> and <a href="#ref-cdc-yrbs">CDC youth surveys</a> for dating and sexual activity, <a href="#ref-nber-sports-betting">Baker et al.</a> and the <a href="#ref-aga-revenue">American Gaming Association</a> for sports betting, <a href="#ref-cdc-upf">CDC/NCHS</a> and <a href="#ref-wilding-semaglutide">Wilding et al.</a> for food and GLP-1 medicines, the <a href="#ref-asps-2024">American Society of Plastic Surgeons</a> for appearance markets, and <a href="#ref-pew-news-influencers">Pew</a>, <a href="#ref-pnas-divisive">PNAS Nexus</a>, <a href="#ref-gallup-institutions">Gallup</a>, and <a href="#ref-pew-trust">Pew Trusts</a> for news influencers, divisive ranking, and institutional trust.</p>
  </section>

  <section>
    <h2>7. Empirical Strategy</h2>
    <p>The model becomes empirical when the capacity stock is named and measured. For loneliness, capacity may include friendship frequency, social anxiety, and offline practice. For betting, it may include savings, credit scores, missed payments, and debt service. For food, it may include metabolic markers and appetite regulation after exposure changes. For politics, it may include institutional trust, correction acceptance, and cross-cutting exposure.</p>
    ...
    <p>The minimal panel test separates within-person or within-place feedback from stable heterogeneity. In words, the researcher needs one equation for how exposure and capacity predict later subjective payoff, one equation for how behavior and subjective payoff predict later capacity, and one behavior equation linking the payoff state to observed substitute use. Appendix D writes these equations explicitly.</p>

    <p>A fixed-preference account is adequate when exposure changes convenience, information, or prices but not the payoff criterion, or when behavior does not feed back into the future capacity that predicts later payoff. A feedback account becomes plausible when exposure changes the payoff state, the payoff state changes behavior, and behavior changes the future capacity that predicts the later payoff state.</p>
  ...
  <section>
    ...
    <p>The contribution is the timing discipline. The payoff criterion forms quickly. Choice, including Nash equilibrium in strategic settings, occurs inside that induced criterion. Behavior then changes a slower material capacity. Finally, preference-forming rules compete under an explicit score. This order lets the same framework describe harmful loops and repair loops without calling preferences fake and without treating competition as automatically welfare improving.</p>
  </section>

  <section>
    <h2>9. What The Result Distinguishes</h2>
    <p>The model does not conclude that people should want different things. It maps which changes alter objective dynamics. A change can work by lowering inducement, lowering sensitivity, reducing damage, strengthening repair, making capacity itself reduce the substitute's pull, moving the state across a threshold, or changing the competitive score. These are comparative statics, not moral categories.</p>

    <p>Technology is therefore not classified by its surface form. An AI companion, recommender, medical intervention, school, workplace, or political institution is a bridge when its induced payoff leads back into durable material capacity. It is a sink when it wins the local payoff game while reducing the capacity that would make alternatives feasible later. Competition can reward either form depending on whether the competitive score is material viability or a proxy such as engagement.</p>

    <p>The model also distinguishes weak optimism from a real change in dynamics. Pure warnings below a threshold need not move the state, because the low-capacity equilibrium can absorb the warning into the same payoff criterion that created the trap. Correction occurs only when the forces pushing capacity up or down change, the state crosses the threshold, or the competitive process reallocates mass toward a rule with higher material viability.</p>
  </section>
  ...
  <section>
    ...
    <p>Fast subjective payoff formation plus slow capacity feedback yields self-correction, threshold traps, or collapse-prone dynamics in the scalar model. An instantaneous current-drift signal alone does not remove a bad interior steady state. Persistent capacity feedback can remove the low trap in the diagnostic calibration. Competition can also shift composition toward the material-viability rule, but only when the competitive score tracks material viability. Nash equilibrium pins down the strategic step after subjective payoffs are formed. Competition pins down the long-run step after the competitive score and material growth rate are named. The same mathematics can describe harmful loops and repair loops without pretending that every current problem has the same cause.</p>
  </section>

  <section>
    <h2>Appendix A. General Formal Model</h2>
    <h3>A.1 Primitives</h3>
    <p>There is a finite set of players \(I=\{1,\ldots,n\}\). Player \(i\) has action set \(A_i\), and \(A=\prod_i A_i\). The slow state is \(K\in\mathcal K\). In applications \(K\) may be scalar or vector-valued. The slow environment is \(E\). The preference-forming rule is \(\ell\). Exposure or input is \(z\). Player \(i\)'s preference state is \(\theta_i\), and subjective utility is \(U_i(a;\theta_i,K,E,\ell)\). The material evaluator is \(G(K,a,E,\ell)\), or player-specific material payoff \(\pi_i(a,K,E,\ell)\) when needed. The material law of motion is \(F(K,x,E,\ell,z)\); it describes how equilibrium behavior changes the slow capacity state. The evaluator \(G\) ranks consequences, while \(F\) moves the capacity stock.</p>
    ...
    <p>The slow state then changes according to a material law. The displayed theorem uses a selected continuous equilibrium branch. If the induced game has several equilibria and no branch is selected, the right-hand side should be treated as a correspondence or differential inclusion with additional assumptions; the paper does not need that generality for the scalar applications.</p>
    ...
    <h3>A.4 Competition And Selection Over Rules</h3>
    <p>Let \(s_\ell\) be the share of aggregate mass \(N\) governed by preference-forming rule \(\ell\), with \(s_\ell\ge0\) and \(\sum_\ell s_\ell=1\). Let \(S_\ell\) be the score that determines relative competition among rules, and let \(g_\ell\) be the material growth, reproduction, persistence, or institutional growth rate of the mass governed by \(\ell\). Let \(\omega\ge0\) be the intensity of competition across rules, and let \(N\) be total population mass, capital mass, or institutional scale. The convention used here separates relative prevalence from aggregate material scale: \(S\) governs shares, while \(g\) governs \(N\).</p>

    <div class="equation">
      <span class="label">Selection over preference-forming rules</span>
      \[
        \dot s_\ell=\omega s_\ell\left(S_\ell-\sum_r s_rS_r\right),
        \qquad
        \dot N=N\sum_r s_rg_r
      \]
    </div>

    <p>This equation is not an additional welfare assumption. It is a way to state the Darwinian question after the competitive score and material growth rate have been named. Relative competition can select the rule with the highest \(S_\ell\), while absolute survival depends on the average \(\sum_r s_rg_r\). If all available rules imply negative material growth, the limiting result may be disappearance of the population, institution, or practice rather than survival of a better preference type.</p>

    <div class="theorem" data-kind="Proposition">
      <span class="theorem-title">Competition selects the metric, not objective welfare by definition</span>
  ...
  <section>
    ...
    <p>The scalar model used for the numerical diagnostics is the special case in which the fast subsystem directly yields a substitute share \(p(K,z)\) and the slow law is one-dimensional. The form is deliberately austere. It is the smallest normal form that allows self-correction, a threshold trap, and collapse-prone capacity loss.</p>
    ...
    <p>To test whether reality corrects the loop automatically, add a current-drift signal to the fast subjective payoff. Let \(\chi\ge0\) measure how salient current material improvement or deterioration is:</p>

    <div class="equation">
      <span class="label">Current-drift reality signal</span>
      \[
        p_\chi(K)=\sigma\{\beta[q+z-\rho K+\chi\Phi_\chi(K)]\},
        \qquad
        \Phi_\chi(K)=\alpha+rK^2(1-K)-dK-Lp_\chi(K)
      \]
    </div>
    ...
    <p>The baseline capacity-trap calibration used for Table 1 and Figures 1-2 sets \(\alpha=0.235\), \(r=2.90\), \(d=0.66\), \(L=0.198\), \(\beta=21.0\), \(q=0.51\), and \(\rho=0.855\). The self-correcting scenario lowers \(L\) and \(\beta\). The repair scenario raises \(\alpha\). The current-drift and competition audits are generated by `scripts/run_self_correction_analysis.py`; the material-feedback baseline is generated by `scripts/run_material_feedback_analysis.py`.</p>
  </section>

  <section>
    <h2>Appendix C. Numerical Diagnostics</h2>
    <p>The figures and tables in this appendix are mechanism diagnostics. They are not estimates of population frequencies. A negative local slope means the state is stable; a positive local slope marks a threshold. Rounded substitute shares shown as 1.000 are very close to one, not assumed exactly equal to one.</p>
    ...
    <p class="small">Entries are shares of parameter draws. The audit uses seed 20260626 and 3,000 draws per feedback class.</p>
    $audit_table

    <p class="table-title">Audit design.</p>
    <p class="small">Classes differ only in substitute damage \(L\), sensitivity \(\beta\), and the capacity-protection channel \(\rho\). Shared ranges are \(\alpha\in[0.10,0.28]\), \(r\in[1,4]\), \(d\in[0.22,0.78]\), \(q\in[0.35,0.65]\), and \(z\in[-0.05,0.10]\).</p>
    <table>
      ...
      <tbody>
        ...
        <tr><td>Trap-prone feedback</td><td>[0.12, 0.24]</td><td>[10, 24]</td><td>[0.70, 1.20]</td><td>Intermediate damage and a sharper capacity-protection channel.</td></tr>
    ...
    <p class="table-title">Stock-feedback correction threshold.</p>
    <p class="small">The sweep varies only the protective effect of capacity on payoff formation, \(\rho\), holding the baseline trap calibration fixed. The low-capacity trap disappears once \(\rho\) is between 3.0 and 3.5 on the grid used here.</p>
    $stock_summary_table

    <p class="table-title">Competition audit.</p>
    <p class="small">The bridge and sink rules are simulated from the same initial capacity and equal shares. Competition intensity \(\omega=0\) is the no-competition control; higher \(\omega\) speeds selection. Population mass is normalized to one initially.</p>
    $competition_table
  </section>

  <section>
    <h2>Appendix D. Empirical Specification</h2>
    <p>Let \(\theta_{it}\) be a measured or latent subjective payoff state, \(K_{it}\) a material capacity, \(z_{it}\) exposure, \(x_{it}\) observed substitute behavior, and \(W_{it}\) prices, income, constraints, and other controls. A minimal panel representation is:</p>
    ...
    <p>The fixed-preference account is adequate when the exposure and capacity feedback terms are small or disappear after controls and stable heterogeneity are included. The feedback account is stronger when exposure predicts the payoff state, the payoff state predicts behavior, and behavior predicts the future capacity that then predicts later payoff.</p>
  </section>
...
def main() -> None:
    if not (ROOT / "results" / "tables" / "material_feedback_equilibria.csv").exists():
        ...
        raise SystemExit(1)
    if not (ROOT / "results" / "tables" / "self_correction_competition_summary.csv").exists():
        print("Run scripts/run_self_correction_analysis.py first.", file=sys.stderr)
        raise SystemExit(1)
