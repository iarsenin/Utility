...
def write_mean_field_hysteresis_svg(
    ...
    stable_low_points: list[tuple[float, float]] = []
    stable_high_points: list[tuple[float, float]] = []
    unstable_points: list[tuple[float, float]] = []
    for field in fields:
        points = mean_field_fixed_points(beta, social_feedback, field)
        stable = [point for point in points if point.stable]
        if len(stable) == 1:
            target = stable_low_points if stable[0].adoption < 0 else stable_high_points
            target.append((field, stable[0].adoption))
        elif len(stable) >= 2:
            ordered = sorted(stable, key=lambda point: point.adoption)
            stable_low_points.append((field, ordered[0].adoption))
            stable_high_points.append((field, ordered[-1].adoption))
        for point in points:
            if not point.stable:
                unstable_points.append((field, point.adoption))
    ...
    elements = [
        ...
        *chart_header(
            "Mean-Field Preferences Can Remember a Shock",
            f"m = tanh(beta(Jm+b)); beta={beta:.1f}, J={social_feedback:.1f}, baseline field h={material_field:.2f}",
            width,
        ),
    ]
    for tick in [-0.3, -0.15, 0.0, 0.15, 0.3]:
        ...
        elements.append(svg_text(x, height - 52, f"{tick:.2f}", font_size=12, fill=CHART_MUTED, text_anchor="middle"))
    ...
    elements.append(polyline(stable_low_points, CHART_TEAL, 3.0))
    elements.append(polyline(stable_high_points, CHART_TEAL, 3.0))
    ...
    elements.append(svg_text(width / 2, height - 38, "total fashion field b = h + z", font_size=13, fill=CHART_INK, text_anchor="middle"))
    elements.append(svg_text(30, height / 2, "settled taste m", font_size=13, fill=CHART_INK, transform=f"rotate(-90 30 {height/2})", text_anchor="middle"))
    elements.append(f'<rect x="616" y="96" width="224" height="92" fill="#ffffff" stroke="{CHART_GRID}"/>')
    elements.append(svg_text(628, 118, "stable settling branches", font_size=12, fill=CHART_TEAL, font_weight=800))
    elements.append(svg_text(628, 139, "unstable separator", font_size=12, fill=CHART_AMBER, font_weight=800))
    elements.append(svg_text(628, 160, "shock arrives", font_size=12, fill=CHART_BLUE, font_weight=800))
    elements.append(svg_text(628, 181, "shock removed", font_size=12, fill=CHART_VIOLET, font_weight=800))
    elements.append(chart_footer("Source: stylized mean-field simulation; see Appendix B and replication scripts.", width, height))
    elements.append("</svg>")
    path.parent.mkdir(parents=True, exist_ok=True)
    path.write_text("\n".join(elements) + "\n", encoding="utf-8")
...
def write_network_multiplier_svg(path: Path, rows: list[NetworkMultiplierRow]) -> None:
    ...
    elements = [
        *svg_open(width, height),
        *chart_header(
            "Influence Multiplier Near a Social-Feedback Threshold",
            "Zero-taste benchmark specialization of the local multiplier, so D = I",
            width,
        ),
    ]
    for tick in [0.0, 0.2, 0.4, 0.6, 0.8]:
        x = sx(tick)
        elements.append(f'<line x1="{x:.1f}" y1="{margin_top}" x2="{x:.1f}" y2="{height-margin_bottom}" stroke="{CHART_GRID}"/>')
        elements.append(svg_text(x, height - 52, f"{tick:.1f}", font_size=12, fill=CHART_MUTED, text_anchor="middle"))
    ...
    elements.append(svg_text(650, 122, "central target", font_size=13, fill=CHART_BLUE, font_weight=800))
    elements.append(svg_text(650, 146, "peripheral target", font_size=13, fill=CHART_AMBER, font_weight=800))
    elements.append(svg_text(width / 2, height - 38, "social reinforcement eta", font_size=13, fill=CHART_INK, text_anchor="middle"))
    elements.append(svg_text(30, height / 2, "aggregate response", font_size=13, fill=CHART_INK, transform=f"rotate(-90 30 {height/2})", text_anchor="middle"))
    elements.append(chart_footer("Source: 120-node simulated attention network; zero-benchmark local derivative, not a finite-shock forecast.", width, height))
    elements.append("</svg>")
    path.parent.mkdir(parents=True, exist_ok=True)
    path.write_text("\n".join(elements) + "\n", encoding="utf-8")
...
def write_phase_audit_svg(path: Path, summary: PhaseAuditSummary) -> None:
    rows = [
        ("Subcritical unique settling", summary.subcritical_unique_rate, CHART_TEAL),
    ...
    margin_top = 100
    row_height = 62
    height = margin_top + row_height * len(rows) + 98
    chart_width = width - margin_left - margin_right
    elements = [
        *svg_open(width, height),
        *chart_header(
            "Random Parameter Check",
    ...
    for tick in [0.0, 0.25, 0.5, 0.75, 1.0]:
        ...
        elements.append(f'<line x1="{x:.1f}" y1="{margin_top-16}" x2="{x:.1f}" y2="{height-72}" stroke="{CHART_GRID}"/>')
        elements.append(svg_text(x, height - 58, f"{tick:.2f}".rstrip("0").rstrip("."), font_size=12, fill=CHART_MUTED, text_anchor="middle"))
    ...
    elements.append(svg_text(width / 2, height - 40, "rate", font_size=13, fill=CHART_INK, text_anchor="middle"))
    elements.append(chart_footer("Source: randomized stylized parameter check; rates are mechanism checks, not empirical estimates.", width, height))
    elements.append("</svg>")
    path.parent.mkdir(parents=True, exist_ok=True)
    path.write_text("\n".join(elements) + "\n", encoding="utf-8")
