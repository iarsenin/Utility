...
This paper studies the extreme logical case. Let `theta_t` denote the preference
...
preference adaptation occurs on timescale `T_pref`. What happens as:
...
law, then the economy no longer evolves with `theta` as an independent slow
...
First, in the `T_pref -> 0` limit with a unique attracting preference manifold,
...
`T_pref -> 0`. The speeds of institutions, algorithms, action adjustment,
...
```text
T_pref d theta / dt = G(theta, z, a, m).
```
...
```text
T_m d m / dt = R(m, z, theta, a).
```

Population or survival dynamics may evolve at another speed:

```text
T_pop d n / dt = n g(z, theta, a, m).
```

The key modeling discipline is that only `T_pref` is sent to zero. The other
...
T_m d m / dt   = R(m, z, Phi(z, m), BR(Phi(z, m), z, m))
T_pop d n / dt = n g(z, Phi(z, m), BR(Phi(z, m), z, m), m).
```
...
`U_i(a, z, theta_i, m)` be subjective utility. A material-payoff Nash
equilibrium satisfies:

```text
a_i in argmax_b pi_i(b, a_-i, z, m).
```

A fast-adapted preference Nash equilibrium satisfies:

```text
a_i in argmax_b U_i(b, a_-i, z, Phi_i(z, m), m).
```

These two objects coincide only under an invariance condition:

```text
argmax_b U_i(b, a_-i, z, Phi_i(z, m), m)
=
argmax_b pi_i(b, a_-i, z, m)
```
...
then inherited variation in `theta` is erased before selection operates.
Selection over preference states becomes degenerate. Selection instead acts on
whatever remains heterogeneous:

```text
adaptation law j -> Phi_j(z, m) -> g_j(z, Phi_j(z, m), m).
```
...
by the path being evaluated. A mechanism can move the agent from `theta_0` to
`theta_1`, allocate `x_1`, and then point to the fact that `theta_1` endorses
`x_1`.
...
where `theta = 0` represents an offline or embodied good and `theta = 1`
represents an online or artificial good. Given current `theta`, the agent solves
a Cobb-Douglas attention problem:

```text
max_a theta log a + (1 - theta) log(1 - a),
```
...
```text
dot theta = exposure(theta, m) (1 - theta) - anchor theta.
```

In the fast limit, `theta` satisfies:

```text
exposure(theta, m) (1 - theta) = anchor theta.
```
...
Material growth is assumed to decline at high `theta`:

```text
g(theta) = baseline + offline_growth (1 - theta) - high_theta_penalty theta^2.
```
...
```text
platform_value = theta - exposure_cost * m^2.
```

The survival-aware institution additionally values material growth:

```text
platform_value = theta - exposure_cost * m^2 + survival_weight * g(theta).
```
...
```text
produce theta_1, satisfy theta_1, cite theta_1 as welfare evidence.
```
...
1. Initial-preference welfare: evaluate the path using `theta_0`.
...
3. Meta-preference welfare: evaluate whether the transition
   `theta_0 -> theta_1` is endorsed by higher-order preferences.
4. Material or survival welfare: evaluate `g(theta, z, m)`.
...
```text
T_pref d theta / dt = G(theta, z, BR(theta, z, m), m)
```

has a globally attracting, normally hyperbolic branch `theta = Phi(z, m)`.
Then as `T_pref -> 0`, solutions converge after an initial boundary layer to
...
`T_pref -> 0` limit. If adaptation laws differ by type `j`, selection acts on:

```text
j -> Phi_j(z, m) -> g_j(z, Phi_j(z, m), m).
```
...
```text
A preferred to B by initial preferences,
B preferred to A by final preferences,
A and B ranked differently by material growth.
```
