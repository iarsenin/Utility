...
- `when_preferences_move_faster_than_equilibrium_v1.html`: current integrated
  manuscript. It is now generated by
  `scripts/build_material_feedback_article.py` and centers the
  material-capacity feedback model: fast subjective payoff formation, ordinary
  Nash equilibrium or choice under the induced payoff, slow capacity dynamics,
  and long-run selection over rules and institutions.
...
Current material-feedback article artifacts:
...
`paper/when_preferences_move_faster_than_equilibrium_v1.html` as a local file
should not produce a MathJax CDN connection error. If the in-app browser still
shows stale content, reload the tab.
