...
<main>
  ...
  <section class="abstract">
    ...
    <p>A second result concerns objective correction. A signal that merely reports current deterioration does not remove the low-capacity steady state, because at a steady state the deterioration signal is zero. Durable correction in the diagnostics comes from the capacity-protection channel or from competition under a material competitive score. Competition is not magic: if the competitive score is material viability, a capacity-preserving rule expands; if the competitive score is engagement, a high-engagement sink can win while material capacity and population mass fall.</p>
    <p>The framework gives a disciplined test for candidate applications such as AI companions, dating retreat, online betting and debt, ultra-processed food, appetite-changing medicines, appearance arms races, and political outrage. It does not claim that these phenomena have one cause. It asks: what forms the subjective payoff, what substitute behavior is chosen, what material capacity is changed, what competitive score selects among systems, and does the loop build capacity, deplete it, or trap the system below a threshold?</p>
  </section>
  ...
  <section>
    ...
    <div class="result-list">
      <h3>Results in the draft</h3>
      <ol>
        <li><strong>Formal reduction.</strong> If preference formation is fast, initial conditions lie in the basin of a selected attracting branch, and no branch switch occurs on the interval, Nash equilibrium is computed in the induced subjective game, and material capacity then moves under the selected equilibrium behavior.</li>
        <li><strong>Scalar capacity result.</strong> The one-stock model has three regimes: a single high-capacity state, two stable states separated by a threshold, or collapse toward a boundary when damage overwhelms repair.</li>
        <li><strong>Correction result.</strong> A current-deterioration signal does not move steady states; persistent material-state feedback can remove the low trap.</li>
        <li><strong>Competition result.</strong> Competition selects the rule with the higher competitive score \(S_\ell\). Absolute survival depends on material growth \(g_\ell\). If \(S_\ell\) is material viability, a bridge can expand. If \(S_\ell\) is engagement, a sink can win while material mass falls.</li>
        <li><strong>Empirical status.</strong> The applications are candidate tests. They require measured exposure, behavior, capacity, and competitive score before they become evidence.</li>
      </ol>
    </div>
    ...
    <div class="result-strip">
      ...
      <div><strong>Competition</strong><span>The long-run question: which rules expand, which disappear, and what metric decides?</span></div>
    </div>
  ...
  <section>
    ...
    <p class="small">The audit uses seed 20260626 and 3,000 draws per class. Classes differ only in substitute damage \(L\), sensitivity \(\beta\), and the capacity-protection channel \(\rho\). Shared ranges are \(\alpha\in[0.10,0.28]\), \(r\in[1,4]\), \(d\in[0.22,0.78]\), \(q\in[0.35,0.65]\), and \(z\in[-0.05,0.10]\).</p>
    <table>
    ...
    <p>No, not in the weak sense of "people eventually notice the damage." If the subjective payoff includes a signal of current material deterioration, the motion can be damped, but the steady states are unchanged. At any steady state, current deterioration is zero, so that signal has nothing left to say. The mechanical result is that warnings and discomfort are not a restoring force unless they remain connected to an enduring material state or a competitive selection process.</p>

    <p>The capacity-protection channel is different. When the level of capacity itself changes the payoff map, a sufficiently strong \(\rho\) can remove the low trap. In the baseline calibration, the low-capacity trap disappears when \(\rho\) is raised to about 3.1 on the reported grid. That is self-correction in the formal sense: the vector field no longer contains the low attracting state.</p>

    <figure>
      <img src="../results/figures/self_correction_channels.svg" alt="Self-correction channel diagnostics">
      <figcaption>Two correction channels. A current deterioration signal changes slopes but leaves the same roots. A persistent capacity-stock signal can change the root pattern and remove the low-capacity trap. The distinction matters because transient regret or pain often behaves like the first channel, while institutions, skills, balance sheets, health, and social networks behave like the second.</figcaption>
    </figure>

    <p class="table-title">Stock-feedback correction threshold.</p>
    <p class="small">The sweep varies only the protective effect of capacity on payoff formation, \(\rho\), holding the baseline trap calibration fixed. The table is a mechanism diagnostic, not an estimate.</p>
    $stock_summary_table

    <h3>Competition Is Corrective Only Under The Right Metric</h3>
    <p>Competition is Darwinian, but Darwinian does not mean benevolent. In the audit below, a bridge rule and a sink rule start at the same capacity and equal shares. Under material-viability competition, the bridge expands and total population mass recovers. Under engagement-proxy competition, the sink expands and material population mass falls. The same replicator equation does both; the difference is the competitive score \(S_\ell\) and its relation to material growth \(g_\ell\).</p>

    <figure>
      <img src="../results/figures/competition_selection_channels.svg" alt="Competition selection channels">
      <figcaption>Competition over rules. When the competitive score is material viability, the capacity-preserving bridge rule takes over. When the competitive score is engagement, the high-substitute sink wins even though material population mass collapses. The figure separates relative victory from absolute survival.</figcaption>
    </figure>
  ...
  <section>
    ...
    <p>The threshold result says that small changes may have different effects depending on the starting stock. Above the threshold, synthetic interaction can be a scaffold that reduces anxiety and returns the user to human contact with higher capacity. Below the threshold, the same local reward can stabilize retreat if outside contact becomes too costly relative to the induced payoff. The current-deterioration result is narrower than "loneliness disappears": loneliness may remain, but the marginal signal that current behavior is making the capacity stock worse can vanish once the system settles into a low-capacity state.</p>
  ...
  <section>
    ...
    <p>The same mechanism can be tested elsewhere, but the following table is a research menu, not a claim that one model explains everything. Each row must name a substitute, a measurable capacity, and a competitive score before it becomes evidence.</p>
  ...
  <section>
    <h2>8. What The Result Rules In And Out</h2>
    <p>The model does not conclude that people should want different things. It maps which changes alter the objective dynamics. A change can work by lowering exposure or inducement \(z\), lowering sensitivity \(\beta\), lowering damage \(L\), raising baseline or practice repair \(\alpha\) or \(r\), strengthening the capacity-protection channel \(\rho\), or changing the competitive score \(S_\ell\). These are comparative statics, not moral categories.</p>

    <p>The capacity-protection channel \(\rho\) has a dual role. A higher \(\rho\) can lower substitute appeal in the high-capacity region. But moderate increases can also sharpen the gap between low and high states, making thresholds more pronounced. Full self-correction appears only when the low attracting state disappears or when competition reallocates prevalence to a rule with higher material-viability score.</p>

    <p>Technology is therefore not classified by its surface form. An AI companion, recommender, medical intervention, school, workplace, or political institution is a bridge when its induced payoff leads back into durable material capacity. It is a sink when it wins the local payoff game while reducing the capacity that would make alternatives feasible later. Competition can reward either form depending on whether the competitive score \(S_\ell\) is material viability or a proxy such as engagement.</p>

    <p>The model also rules out a weak optimism. Pure warnings below a threshold need not move the state, because the low-capacity equilibrium can absorb the warning into the same payoff criterion that created the trap. In the model, correction occurs only when the vector field changes, the state crosses the threshold, or the competitive process reallocates mass toward a rule with higher material viability.</p>
  </section>
  ...
  <section>
    ...
    <p>The model turns that observation into a tractable object. Fast subjective payoff formation plus slow capacity feedback yields self-correction, threshold traps, or collapse-prone dynamics. A current deterioration signal alone does not generally remove a bad steady state. The capacity-protection channel can. Competition can also correct the composition of rules, but only when the competitive score tracks material viability. Nash equilibrium disciplines the strategic step after subjective payoffs are formed. Competition disciplines the long-run step after the competitive score and material growth rate are named. The same mathematics can describe harmful loops and repair loops without pretending that every current problem has the same cause.</p>
  </section>
  ...
  <section>
    ...
    <div class="theorem" data-kind="Proposition">
      <span class="theorem-title">Current deterioration does not move steady states</span>
