...
<main>
  ...
  <section class="abstract">
    ...
    <p>The capacity loop has three steps plus a selection layer. First, exposure and current capacity form a fast preference state or payoff map. Second, agents choose coherently inside that induced game, with Nash equilibrium as the strategic consistency condition in strategic settings. Third, behavior moves the slower capacity stock. Finally, payoff-forming rules compete under a named score. The key feedback chain is capacity to payoff, payoff to behavior, and behavior back to capacity.</p>
    <p>In the one-capacity smooth-choice diagnostic, this loop produces three long-run patterns. With weak damage or strong repair, capacity recovers. In the baseline trap calibration, there is a low-capacity state, a high-capacity state, and a threshold between them; on the normalized capacity scale those points are about 0.085, 0.843, and 0.212. In the higher-damage audit, the collapse-prone class reaches the lower boundary in 88.5 percent of draws and has no interior stable state in 15.4 percent. In all patterns, choice can remain Nash-consistent after subjective payoffs are formed.</p>
    <p>The correction result is sharper than the usual intuition that current feedback eventually corrects behavior. A signal tied only to whether capacity is currently rising or falling changes speeds and slopes, but it does not move interior steady-state capacities; once the system is at rest, that signal is silent. A sufficiently strong level-of-capacity channel can move the steady states and remove the low trap. Competition can also favor repair, but only when the competitive score ranks the capacity-preserving rule above the sink. If the score rewards engagement instead, the sink can gain relative share while aggregate material scale falls.</p>
